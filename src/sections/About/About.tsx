...
const About = () => {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(imgRef.current,
      { filter: "blur(15px) grayscale(50%)", opacity: 0.5 },
      { filter: "blur(0px) grayscale(0%)", opacity: 1, duration: 0.5, ease: "power2.out" },
      "<"
    )
  ...
  return (
    <section id="about" ref={containerRef} className="about-section relative min-h-screen pt-20">
      <div className="flex flex-col lg:flex-row items-center min-h-[calc(100vh-80px)] gap-12 lg:gap-16 w-full px-[8vw] md:px-[12vw] lg:mb-0 mb-20 lg:h-full">
        <div className="flex flex-col md:flex-row justify-center items-center md:items-center gap-8 lg:gap-12 w-full lg:flex-1">
          <div className="flex flex-col items-center min-w-[140px] md:min-w-[180px] 
            ...
            <div className="mt-4 md:mt-10 text-[0.8rem] uppercase tracking-[0.2em] text-text-main/60 font-medium text-center md:text-left">
              <div className="text-[14px]">{t('about.year')}</div>
              <div>{t('about.devExperience')}</div>
            </div>
            <div className="mt-4 text-[0.85rem] text-text-main/40 font-mono italic">
              {t('common.seniorFrontend')}
            </div>

            <div className="about-paragraph mt-4 inline-flex items-center gap-3 px-4 py-2 border border-black/5 bg-black/2 rounded-sm w-fit mx-auto md:mx-0">
              {/* 在職中的狀態 bg-green-500, 等待機會的狀態 bg-blue-500 */}
              <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
              <span className="text-[0.85rem] font-mono text-text-main/60 uppercase tracking-widest">
